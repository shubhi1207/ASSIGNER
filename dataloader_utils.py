...
class TextDataset(Dataset):
    ...
    def collate_pad(batch, cutoff: int):
        max_token_len = 0
        num_elems = len(batch)
        # batch[0] -> _getitem_[0] --> returns a tuple (embeddings, out_dict)
...
class TextDataset_translation(TextDataset):
    ...
    def read_data(self):
        ...
        src_lines = open(self.data_path + '.' + self.src, 'r').readlines()
        tgt_lines = open(self.data_path + '.' + self.tgt, 'r').readlines()
        
        # Read sentiment data and convert to arrays of floats
        # with open(self.data_path + '.se', 'r') as file:
            # sentiment_lines = [np.fromstring(line.strip('[]\n'), sep=', ') for line in file.readlines()]
        with open(self.data_path+'_selector8_embeddings.pkl','rb') as f:
            ret_embeddings = pickle.load(f)    
        print(f"Tokenizing {len(src_lines)} sentences")
        # Combine the data into a single list
        data = [[src, tgt,  ret] for src, tgt, ret in zip(src_lines, tgt_lines, ret_embeddings)]
        # Extract the source, target, and sentiment texts
        self.src_text = [item[0].strip('\n') for item in data]
        self.tgt_text = [item[1].strip('\n') for item in data]
        # self.sent_text = [torch.tensor(item[2], dtype=torch.float32) for item in data]
        self.ret_text = [torch.tensor(item[2], dtype=torch.float32) for item in data] 
    ...
    def __getitem__(self, i):
        out_dict = {
            ...
            "ret_embeddings":self.ret_text[i]
        }
        return out_dict
    @staticmethod
    def collate_pad(batch, args, cutoff: int, cutoff_src: int, padding_token: int):
        ...
        for i in range(num_elems):
            ...
            tokens_mask_tgt[i, :] = 1
        # sentiment_scores = torch.stack([batch[i]["sentiment_scores"] for i in range(num_elems)])
        padded_ret_embeddings = []
        for item in batch:
          ret_embedding = item["ret_embeddings"]
        # Get the current size
          current_size = ret_embedding.size(0)
        # If the current size is less than max_size, pad with zeros
          if current_size < 8:
            padding = torch.zeros(8 - current_size, ret_embedding.size(1))  # Assuming 2nd dimension is consistent
            padded_ret_embedding = torch.cat([ret_embedding, padding], dim=0)
          else:
            padded_ret_embedding = ret_embedding
          padded_ret_embeddings.append(padded_ret_embedding)
    # Stack the padded tensors
        ret_embeddings = torch.stack(padded_ret_embeddings)
        return {"input_ids": tokens_src, "attention_mask": tokens_mask_src, 
                    'decoder_input_ids': tokens_tgt, 'decoder_attention_mask': tokens_mask_tgt,"ret_embeddings":ret_embeddings}, None
