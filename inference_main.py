...
def main():
    ...
    logger.info(f"Use pretrained tokenizer = {args.use_pretrained_tokenizer}")
    ...
    while len(all_samples) * args.batch_size < args.num_samples:
        ...
        print('number of samples', len(all_samples), all_samples[0].shape)
    ...
    for seq in cands:
        ...
        if seq.ndim > 1:
            seq = seq.squeeze()
        if seq.size > 0:  # Check if seq is not empty
            decoded_sentence = tokenizer.decode(seq.tolist(), skip_special_tokens=True)
        else:
            decoded_sentence = ""  # Handle empty case
    ...
    for seq in ground_true_samples:
        ...
        if seq.ndim > 1:
            seq = seq.squeeze()
        if seq.size > 0:  # Check if seq is not empty
            ground_true_sentence = tokenizer.decode(seq.tolist(), skip_special_tokens=True)
        else:
            ground_true_sentence = ""  # Handle empty case
